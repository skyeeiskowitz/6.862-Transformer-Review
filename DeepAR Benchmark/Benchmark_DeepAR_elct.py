...
from DeepAR.evaluate import evaluate
...
workers = 4 # set to 4 for multiprocessing, 0 for not (i.e. Windows)

def train(model: nn.Module, #
          optimizer: optim, #adaptive learning rate optimization algorithm
          loss_fn, #specified loss function we are using
    ...
    '''
    model.train() #specifies for pytorch that we are training the model
    ...
    for i, (train_batch, idx, labels_batch) in enumerate(tqdm(train_loader)):
        ...
        print(loss_epoch[i])
        if i % 1000 == 0:
            test_metrics = evaluate(model, loss_fn, test_loader, params, epoch, sample=True)
            model.train()
...
def train_and_evaluate(model: nn.Module,
    ...
    for epoch in range(params.num_epochs):
        ...
        utils.plot_all_epoch(ND_summary[:epoch + 1], 'solar' + '_ND', params.plot_dir)
        utils.plot_all_epoch(loss_summary[:(epoch + 1) * train_len], 'solar' + '_loss', params.plot_dir)
